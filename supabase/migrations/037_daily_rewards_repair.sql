-- Repair migration for daily rewards/referrals.
-- 036 created these objects, but PostgREST can retain a stale schema cache
-- after functions are added. This migration is idempotent and explicitly
-- asks PostgREST to reload its schema.
--
-- It also moves referral-code creation into a SECURITY DEFINER function so
-- RLS cannot silently prevent a kid from receiving their own invite code.
...
create policy "daily_spins_select_own" on public.daily_spins
  for select using (auth.uid() = user_id);

drop policy if exists "streak_reward_claims_select_own" on public.streak_reward_claims;
create policy "streak_reward_claims_select_own" on public.streak_reward_claims
  for select using (auth.uid() = user_id);
...
begin
  ...
  loop
    v_candidate := upper(substr(md5(gen_random_uuid()::text), 1, 8));
    begin
      update public.users
    ...
    exception when unique_violation then
      -- Extremely unlikely collision; generate another candidate.
    end;
  end loop;
end;
$$;
...
begin
  ...
  if exists (
    select 1 from public.streak_reward_claims
    where user_id = p_user_id and streak_milestone = p_milestone
  ) then
    return null;
  end if;
...
begin
  ...
  if found then
    ...
    update public.users set xp = xp + 25 where id = new.user_id;
  end if;
...
grant execute on function public.claim_daily_spin(uuid) to authenticated;
grant execute on function public.claim_streak_reward(uuid, int, int) to authenticated;
grant execute on function public.apply_referral_code(uuid, text) to authenticated;

-- Force PostgREST/Supabase API to see the functions immediately.
